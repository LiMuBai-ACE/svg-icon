...
class Woff2Base64 {
  ...
  fontStyles = ["italic", "oblique", "normal"];
  ...
  constructor(fonts, opts) {
    ...
    if (!this.opts.fontFamily || typeof this.opts.fontFamily !== "string") {
      throw new Error(
        "No valid fontFamily given in options. Please provide a fontFamily (string)."
      );
    }
    ...
    for (let filename in this.fonts) {
      ...
      if (
        typeof this.fonts[filename] === "string" ||
        this.fonts[filename] instanceof Buffer
      ) {
        this.fonts[filename] = {
  ...
  getFontWeight = (filename, font) => {
    return font.hasOwnProperty("weight") &&
      this.fontWeights.hasOwnProperty(font.weight)
      ? font.weight
      : this.getFontWeightFromFilename(filename);
  };
  ...
  getFontStyle = (filename, font) => {
    return font.hasOwnProperty("style") &&
      this.fontStyles.hasOwnProperty(font.style)
      ? font.style
      : this.getFontStyleFromFilename(filename);
  };
